...
if (!Number.isSafeInteger(sourceOrderId) || sourceOrderId <= 0) {
  throw new Error("Usage: npx tsx scripts/remake-songcraft-pronunciation.ts <source-order-id>");
}
...
const remakePaymentId = `admin-remake:${sourceOrderId}:pronunciation-v3`;
...
if (!remake) {
  remake = await prisma.order.create({
    data: {
      ...
      genre: source.genre,
      mood: source.mood,
      voiceType: source.voiceType === "female" ? "female" : "male",
      style: null,
      tempo: source.tempo,
...
if (created) {
  await fetch(`https://api.telegram.org/bot${BOT_TOKEN()}/sendMessage`, {
    ...
    body: JSON.stringify({
      ...
      text: `Бесплатно переделываем заказ №${sourceOrderId}: исправили ударения и сделали подачу более чёткой. Три новые версии автоматически придут сюда после генерации.`,
    }),
  }).catch(() => null);
}
